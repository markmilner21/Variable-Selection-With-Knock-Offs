...
generate_scenarios_predictive = function(scenario, model, sample_size, num_features, predictive_amplitude, predictive_amplitude_high=-1, predictive_amplitude_mod=-1,predictive_amplitude_low=-1) {
  ...
  if(scenario == 'S1')
  {
    ...
    if (model=='M7')
    {
      ...
      t <- rbinom(sample_size, 1, prob_t)
      sigma_z <- toeplitz(rho^(0:(num_features - 1)))
      X <- data.frame(rmvt(sample_size, sigma = sigma_z, df = 3))
      nonzero_pred = seq(1,100,1)
      nonzero_prog = seq(1,100,1)
      prog_part = apply(X[,nonzero_prog],1,sum)
      pred_part = predictive_amplitude*t*(apply(X[,nonzero_pred],1,sum))
  ...
  if(scenario == 'S3')
  {
    ...
    # mark addition - remember high dimensional scenario 
    if(model=='M3')
    {
      nonzero_pred_high = seq(76,100,1)
      non_zero_pred_mod = seq(26,75,1)
      non_zero_pred_low = seq(1,25,1)
      nonzero_prog = seq(1,100,1)
      prog_part =   X[,nonzero_prog[1]] + X[,nonzero_prog[2]] + X[,nonzero_prog[3]] + X[,nonzero_prog[4]] + X[,nonzero_prog[5]]
      if(predictive_amplitude == 0 ){
        mu = 0.5*t + as.numeric(as.matrix((prog_part)))
      ...
      if(predictive_amplitude != 0){
       # pred_part = (X[,nonzero_pred[1]]>0) + (X[,nonzero_pred[2]]<0) + (X[,nonzero_pred[3]]<0.675) + (X[,nonzero_pred[4]]>0.675) + (X[,nonzero_pred[5]]<0.675)
        pred_part_high <- predictive_amplitude_high * t * rowSums(X[, nonzero_prog[1:25]])
        pred_part_mod  <- predictive_amplitude_mod  * t * rowSums(X[, nonzero_prog[26:75]])
        pred_part_low  <- predictive_amplitude_low  * t * rowSums(X[, nonzero_prog[76:100]])
...
#1. S1M6 - S_pred = S_prog, 100 prog variables and 1000 total covariates.
...
# Now we have the functions working 

```{r}
selected_with_cf_filter = causal_forest_predictive_filter(synthetic_data$X, X_tilde,
synthetic_data$y, synthetic_data$t, fdr_nominal = 0.10, family = 'gaussian')
paste('CF returns the variables:',
paste(selected_with_cf_filter, collapse=","))
```

```{r}
power_result_cf = power(selected_with_linear_filter, synthetic_data$predictive)
fdp_result_cf = fdp(selected_with_linear_filter, synthetic_data$predictive )
```
